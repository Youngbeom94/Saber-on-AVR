...
void q1_barrett_ntt(int16_t a[SABER_N])
{
    ...
    for (len = 128; len > 1; len >>= 1)
    {
        for (start = 0; start < SABER_N; start = j + len)
        {
            ...
            zetap = q1_streamlined_CT_table[k++];
            for (j = start; j < start + len; ++j)
            {
                //! if we want stream : t = Barrett_mul_approx(a[j + len], zeta, gethi(zeta), Q);
                t = barrett_mul_approx(a[j + len], zeta, zetap, Q1);
                a[j + len] = a[j] - t;
                a[j] = a[j] + t;
            }
        }
...
void q2_barrett_ntt(int16_t a[SABER_N])
{
    ...
    for (len = 128; len > 0; len >>= 1)
    {
        for (start = 0; start < SABER_N; start = j + len)
        {
            ...
            zetap = q2_streamlined_CT_table[k++];
            for (j = start; j < start + len; ++j)
            {
                //! if we want stream : t = Barrett_mul_approx(a[j + len], zeta, gethi(zeta), Q);
                t = barrett_mul_approx(a[j + len], zeta, zetap, Q2);
                a[j + len] = a[j] - t;
                a[j] = a[j] + t;
            }
        }
...
void q1_barrett_invntt(int16_t a[SABER_N])
{
    ...
    k = 0;
    for (len = 2; len < SABER_N; len <<= 1)
    {
        for (i = 0; i < len; i++)
        {
            zeta  = q1_streamlined_CT_inv_table[k + 0];
            zetap = q1_streamlined_CT_inv_table[k + 1];
            if ((i & 1) == 1) k += 2;                

            for (j = 0; j < SABER_N / (len << 1); j++)
            {
                if (zeta == 1)
                    t = a[j * (len << 1) + i + len]; //! do light Butterfly
                else
                    t = barrett_mul_approx(a[j * (len << 1) + i + len], zeta, zetap, Q1); //! do Origin Butterfly
                                                                                          //! if we want stream : t = Barrett_mul_approx(a[j * (len << 1) + i + len], zeta, gethi(zeta), Q);
                if ((len == 8))
                {
                    a[j * (len << 1) + i + len] = q1_barrett_reduce(a[j * (len << 1) + i] - t);
                    a[j * (len << 1) + i] = q1_barrett_reduce(a[j * (len << 1) + i] + t);
                }
                else
                {
...
void q2_barrett_invntt(int16_t a[SABER_N])
{
    ...
    for (len = 1; len < SABER_N; len <<= 1)
    {
        for (i = 0; i < len; i++)
        {
            ...
            zetap = q2_streamlined_CT_inv_table[k++];
            for (j = 0; j < SABER_N / (len << 1); j++)
            {
                if (zeta == 1)
                    t = a[j * (len << 1) + i + len]; //! do light Butterfly
                else
                    t = barrett_mul_approx(a[j * (len << 1) + i + len], zeta, zetap, Q2); //! do Origin Butterfly
                //! if we want stream : t = Barrett_mul_approx(a[j * (len << 1) + i + len], zeta, gethi(zeta), Q);
                if ((len ==4) || (len == 64))
                {
                    a[j * (len << 1) + i + len] = q2_barrett_reduce(a[j * (len << 1) + i] - t);
                    a[j * (len << 1) + i]       = q2_barrett_reduce(a[j * (len << 1) + i] + t);
                }
                else
                {
                    a[j * (len << 1) + i + len] = (a[j * (len << 1) + i] - t);
